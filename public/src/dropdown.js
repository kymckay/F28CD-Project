import {setSelectedYear} from './graph.js'
...
export function dropdown(className) {
  ...
  l = x.length;
  for (i = 0; i < l; i++) {
    selectedOption = x[i].getElementsByTagName("select")[0];
    ll = selectedOption.length;
    /*for each element, create a new DIV that will act as the selected item:*/
    a = document.createElement("DIV");
    a.setAttribute("class", "select-selected");
    a.innerHTML = selectedOption.options[selectedOption.selectedIndex].innerHTML;
    x[i].appendChild(a);
    ...
    for (j = 1; j < ll; j++) {
      ...
      c.innerHTML = selectedOption.options[j].innerHTML;
