...
export async function initMap(apiKey) {
  ...
  // Include fs module
  const fs = require('fs');
  const constData = fs.readFileSync('data/constituencies.geojson');
  const constName = [];
  const constPoly = [];
  for (const i in constData) {
    constName[i] = JSON.parse(constData[i]).properties.pcon19nm;
    constPoly[i] = JSON.parse(constData[i]).properties.coordinates;
  };
  ...
  map.on('load', function () {
    ...
    //   });
    // });
    while (i < constName.length) {
      map.addSource(constName[i], {
        'type': 'geojson', 
        'data': {
          'type': 'Feature',
          'geometry': {
            'type': 'Polygon',
            'coordinates': constPoly[i]
          }
        }
      });
      map.addLayer({
